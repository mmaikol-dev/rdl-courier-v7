...
import { Check, ChevronsUpDown, FilterIcon, RefreshCwIcon, UserCheck, X } from 'lucide-react';
...
const STATUS_OPTIONS = ['New Orders', 'Pending', 'Delivered', 'Scheduled', 'Dispatched', 'Cancelled', 'Followup', 'WrongContact'];
...
export default function Index() {
    const { props } = usePage();
    const { orders, filters, merchantUsers, ccUsers, productNames } = props as unknown as {
    ...
    const COLUMNS = [...BASE_COLUMNS, 'checkbox'];
    ...
    const applyFilters = () => {
        ...
        const cleanedFilters = Object.fromEntries(Object.entries(localFilters).filter(([, value]) => value !== ''));

        router.get('/assign', cleanedFilters, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const handleReassign = () => {
        if (selectedOrders.length === 0) {
            toast.error('Please select at least one order to reassign');
            setResultMessage({
    ...
    const confirmReassign = () => {
        if (selectedCcAgents.length === 0) {
            toast.error('Please select at least one call center agent');
            setResultMessage({
        ...
        router.post(
            ...
            },
            {
                onSuccess: (page) => {
                    ...
                    const agentText = selectedCcAgents.length === 1 ? selectedCcAgents[0] : `${selectedCcAgents.length} agents (round-robin)`;
                    toast.success(`Reassigned ${selectedOrders.length} order(s) to ${agentText}`);
                    setResultMessage({
                        title: 'Success',
                ...
                onError: (errors) => {
                    ...
                    const errorMessage = (errors as { message?: string })?.message || 'Failed to reassign orders. Please try again.';
    ...
    return (
        <AppLayout breadcrumbs={BREADCRUMBS}>
            ...
            <div className="mb-2 flex justify-between px-4 pt-4">
                <div>
                ...
                <div className="scrollbar-custom overflow-x-auto">
                    <Table className="min-w-full table-fixed border border-gray-200">
                        <TableHeader className="sticky top-0 z-10 bg-background">
                            <TableRow>
                                {COLUMNS.map((col) => (
                                    <TableHead key={col} className="w-[10%] border border-gray-300 text-sm font-medium">
                                        {col === 'checkbox' ? (
            ...
            <Dialog open={resultDialogOpen} onOpenChange={setResultDialogOpen}>
                <DialogContent className="max-w-md">
                    ...
                    <div className="py-4">
                        <p className="text-sm text-muted-foreground">{resultMessage.description}</p>
                    </div>
                ...
                <DialogContent className="max-w-md">
                    ...
                    <div className="mt-4 space-y-4">
                        ...
                        <div className="flex flex-col space-y-2">
                            <label className="text-sm font-medium">Select Call Center Agents (Multi-select)</label>
                                ...
                                <div className="flex flex-wrap gap-2 rounded-md border bg-muted/20 p-2">
                                    {selectedCcAgents.map((agent) => (
                                        <Badge key={agent} variant="secondary" className="pr-1 pl-2">
                                            {agent}
                                            <button onClick={() => removeCcAgent(agent)} className="ml-1 rounded-full p-0.5 hover:bg-muted">
                                                <X className="h-3 w-3" />
                                            </button>
                                        </Badge>
                                    ))}
                            ...
                            <Popover open={reassignCcOpen} onOpenChange={setReassignCcOpen}>
                                <PopoverTrigger asChild>
                                    <Button variant="outline" role="combobox" className="w-full justify-between">
                                        {selectedCcAgents.length === 0 ? 'Select Call Center Agents' : `${selectedCcAgents.length} agent(s) selected`}
                                ...
                                <PopoverContent className="w-[350px] p-0">
                                    <Command>
                                        ...
                                        <CommandList>
                                            <CommandGroup>
                                                {ccUsers.map((cc, idx) => (
                                                    <CommandItem key={idx} value={cc} onSelect={() => toggleCcAgent(cc)}>
                                                        <div className="flex w-full items-center">
                                                            <Checkbox checked={selectedCcAgents.includes(cc)} className="mr-2" />
                                                            {cc}
                                                        </div>
                                                    </CommandItem>
                                                ))}
                    ...
                    <div className="mt-6 flex justify-end gap-2">
                        ...
                        <Button onClick={confirmReassign}>Confirm Reassign</Button>
                    </div>
                </DialogContent>
            </Dialog>
                        ...
                        <div className="mt-4 space-y-4">
                            <div className="flex flex-col space-y-2">
                                ...
                                <Popover>
                                    <PopoverTrigger asChild>
                                        <Button variant="outline" role="combobox" className="w-full justify-between">
                                            {localFilters.country || 'Select Country'}
                                            <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                        </Button>
                                    </PopoverTrigger>
                                                    ...
                                                    {EAST_AFRICAN_COUNTRIES.map((country) => (
                                                        ...
                                                        >
                                                            <Check
                                                                className={cn(
                                                                    'mr-2 h-4 w-4',
                                                                    localFilters.country === country ? 'opacity-100' : 'opacity-0',
                                                                )}
                                                            />
                                                            {country}
                                                        </CommandItem>
                            ...
                            <div className="flex flex-col space-y-2">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setLocalFilters((prev) => ({ ...prev, delivery_date: e.target.value }))}
                                />
                            </div>
                        </div>
