...
export default function DispatchView() {
    ...
    const readOnly = userRole === 'callcenter1';
    ...
    const parsedOrders = React.useMemo(() => parseOrderNumbers(bulkOrderNumbers), [bulkOrderNumbers]);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-6 p-4 md:p-6">
                {/* Flash Messages */}
                {flash?.success && (
                    <Alert className="border-green-200 bg-green-50 duration-300 animate-in fade-in slide-in-from-top-2">
                ...
                {flash?.error && (
                    <Alert variant="destructive" className="duration-300 animate-in fade-in slide-in-from-top-2">
                                ...
                                <PopoverTrigger asChild>
                                    ...
                                    >
                                        ...
                                        {printDateRange.from ? (
                                            printDateRange.to ? (
                                                <>
                                                    {format(printDateRange.from, 'LLL dd, y')} - {format(printDateRange.to, 'LLL dd, y')}
                                                </>
                                            ) : (
                            ...
                            <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
                                <div className="flex items-start gap-2">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        {(!printDateRange.from || !printDateRange.to) && (
                                            <p className="text-xs text-blue-700">All orders (no date filter)</p>
                                        )}
                                    </div>
                                </div>
                            </div>
                ...
                <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
                    ...
                    <div className="space-y-4 py-4">
                        <div className="space-y-2">
                            ...
                            />
                            <p className="text-xs text-muted-foreground">Separate by commas, spaces, or new lines. Maximum 50 orders per batch.</p>
                        </div>
                        ...
                        <div className="relative">
                            ...
                            <div className="relative flex justify-center text-xs uppercase">
                                <span className="bg-background px-2 text-muted-foreground">Or filter by</span>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <Select value={bulkDownloadAgent} onValueChange={setBulkDownloadAgent} disabled={parsedDownloadOrders.length > 0}>
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-muted-foreground">
                                {parsedDownloadOrders.length > 0 ? 'Disabled when order numbers are provided' : 'Leave empty to download all dates'}
                            </p>
                        </div>
                                ...
                                <div className="max-h-[200px] overflow-y-auto rounded-lg border bg-muted/30 p-3">
                                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
                                        {parsedDownloadOrders.map((order, idx) => (
                                            <div key={idx} className="truncate rounded border bg-background px-2 py-1 text-sm" title={order}>
                                                {order}
                                            </div>
                                        ))}
                                    </div>
                        ...
                        {parsedDownloadOrders.length === 0 && (bulkDownloadAgent !== 'all' || bulkDownloadDateRange.from) && (
                            <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
                                <div className="flex items-start gap-2">
                                    <AlertCircle className="mt-0.5 h-5 w-5 flex-shrink-0 text-blue-600" />
                                    <div className="space-y-1">
                                        <p className="text-sm font-medium text-blue-900">Download waybills with filters:</p>
                                        <ul className="space-y-1 text-xs text-blue-700">
                                            {bulkDownloadAgent && bulkDownloadAgent !== 'all' && (
                                                <li>
                                                    • Agent: <strong>{bulkDownloadAgent}</strong>
                                                </li>
                                            )}
                                            {bulkDownloadDateRange.from && bulkDownloadDateRange.to && (
                                                <li>
                                                    • Date Range: {format(bulkDownloadDateRange.from, 'MMM dd')} -{' '}
                                                    {format(bulkDownloadDateRange.to, 'MMM dd, yyyy')}
                                                </li>
                                            )}
                                            {(!bulkDownloadAgent || bulkDownloadAgent === 'all') && <li>• All agents</li>}
                                            {!bulkDownloadDateRange.from && <li>• All dates</li>}
                                        </ul>
                                    </div>
                                </div>
                            </div>
                    ...
                    <DialogFooter className="gap-2">
                        ...
                        <Button onClick={handleBulkDownload} disabled={isBulkDownloading}>
                            {isBulkDownloading ? (
                                <>
                                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                    Downloading...
            ...
            <Dialog open={showBulkAssignModal} onOpenChange={setShowBulkAssignModal}>
                <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>Assign multiple orders to an agent and generate a consolidated waybill PDF.</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        <div className="space-y-2">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="mt-2 flex flex-wrap gap-1">
                                            {parsedOrders.slice(0, 10).map((order, idx) => (
                                                <span
                                                    key={idx}
                                                    className="truncate rounded border border-blue-300 bg-white px-2 py-1 text-xs"
                                                    title={order}
                                                >
                                                    {order}
                                                </span>
                                            ...
                                            {parsedOrders.length > 10 && (
                                                <span className="rounded border border-blue-300 bg-white px-2 py-1 text-xs">
            ...
            <Dialog open={showFilterModal} onOpenChange={setShowFilterModal}>
                <DialogContent className="max-h-[85vh] max-w-md overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>Narrow down orders by search, date range, or assigned agent.</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-5 py-4">
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                className="h-11"
                            />
                            <p className="text-xs text-muted-foreground">Searches across order number, client name, product name, and phone</p>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <Popover>
                                <PopoverTrigger asChild>
                                    <Button
                                        variant="outline"
                                        className={cn('h-11 w-full justify-start text-left font-normal', !dateRange.from && 'text-muted-foreground')}
                                    >
                                        <CalendarIcon className="mr-2 h-4 w-4" />
                                        {dateRange.from ? (
                                            dateRange.to ? (
                            ...
                            <p className="text-xs text-muted-foreground">Leave empty to show all delivery dates</p>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-muted-foreground">Shows orders assigned to a specific agent, or all agents</p>
                        </div>

                        <div className="space-y-2">
                            <Label htmlFor="country_filter" className="text-base font-semibold">
                                Country
                            </Label>
                            <Select value={selectedCountry} onValueChange={setSelectedCountry}>
                                <SelectTrigger id="country_filter" className="h-11">
                            ...
                            <p className="text-xs text-muted-foreground">Narrow dispatch orders to one East African country</p>
                        </div>

                        {hasActiveFilters && (
                            <div className="rounded-lg border border-orange-200 bg-orange-50 p-4">
                                <div className="flex items-start gap-2">
                                    <Filter className="mt-0.5 h-5 w-5 flex-shrink-0 text-orange-600" />
                                    <div>
                                        ...
                                        <ul className="mt-1 space-y-1 text-xs text-orange-700">
                                            ...
                                            {selectedCountry !== 'all' && <li>Country: {selectedCountry}</li>}
                                        </ul>
                                    </div>
                                </div>
                            </div>
                    ...
                    <DialogFooter className="gap-2">
                        <Button variant="outline" onClick={handleClearFilters} disabled={!hasActiveFilters || isFiltering}>
                            Clear All
                        </Button>
            ...
            <Dialog open={!!editingOrder} onOpenChange={(open) => !open && setEditingOrder(null)}>
                <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>Update order details. Changes will be reflected immediately.</DialogDescription>
                    </DialogHeader>
                    {editingOrder && (
                        <div className="space-y-4 py-4">
                ...
                <DialogContent className="max-h-[85vh] max-w-md overflow-y-auto">
                    ...
                    <div className="space-y-2 py-2 text-center">
                        <p className="text-sm text-muted-foreground">
                            Are you sure you want to delete order <strong className="text-foreground">{deletingOrder?.order_no}</strong>?
                        </p>
                        <p className="text-xs text-muted-foreground">
                    ...
                    <DialogFooter className="gap-2 sm:justify-center">
                        <Button variant="outline" onClick={() => setDeletingOrder(null)} disabled={isDeleting}>
                            Cancel
                        </Button>
